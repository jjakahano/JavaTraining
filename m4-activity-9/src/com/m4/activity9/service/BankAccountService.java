...
public class BankAccountService {
	...
	public void createAccount(String accountNumber, String ownerName, double initialBalance)
		...
		if (initialBalance < MINIMUM_INITIAL_BALANCE) {
			...
			throw new InvalidAccountException(String.format("Initial balance cannot be less than minimum P%f, requested Initial balance: P%f",
		...
		if (accountExists(accountNumber)) {
			...
			logger.warn("Account already exists: accountNumber: {}",accountNumber);
			// TODO 10: Throw InvalidAccountException with message "Account number already
			// exists: " + accountNumber
			throw new InvalidAccountException("Account already exists: accountNumber: " + accountNumber);
		}
		...
		try {
			...
			logger.error("Failed to create account due to Invalid Arguments with {}", e.getMessage());
	...
	public void transfer(String fromAccountNumber, String toAccountNumber, double amount)
		...
		if (amount <= 0) {
			...
			throw new InvalidTransferException(String.format("Transfer amount must be positive. Received amount: %f", amount));
		}
		...
		if (fromAccount == null) {
			...
			// Parameter: fromAccountNumber
			logger.warn("Transfer failed: source amount {} not found", fromAccountNumber);
		...
		if (fromAccount.getBalance() < amount) {
			...
			throw new InsufficientFundsException(String.format("Balance: %f, Requires: %f", fromAccount.getBalance(), amount));
		...
		try {
			...
			// original exception
			// Pass exception 'e' as second parameter
			throw new InvalidTransferException("Transfer failed: " + e.getMessage());
	...
	public List<BankAccount> getAllAccounts() {
		...
		logger.debug("Retrieving all acounts, counts: {}", accounts.size());
